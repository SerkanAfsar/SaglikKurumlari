...
export default function CityListWrapper({ cityList }: { cityList: string[] }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <input
        className="bg-primary mx-auto inline-block w-[240px] rounded-md p-3 text-sm text-white outline-none placeholder:text-white"
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {result.map((item, index) => (
          <Link
            ...
            className="text-primary flex h-full min-h-32 items-center justify-center gap-2 rounded-md bg-white p-6 text-center text-lg font-bold uppercase transition-all hover:scale-105 hover:shadow"
            key={index}
          >
